...
## ✨ Novidades da v2.0 | v2.0 Highlights
...
**Correções:** erro de compilação corrigido (chave extra no bloco de volume); LEDs que ficavam apagados após reflash (causa: `ledcAttach` falhando silenciosamente).
...
**Fixes:** fixed compile error (extra brace in the volume block); fixed LEDs staying off after reflashing (cause: `ledcAttach` failing silently).